#Detect Respawns
execute as @a[scores = {lh_deathInit = 1..}, nbt = !{Health: 0f}] run function lighthard:respawn

#You cannot use milk to cure death effects
execute as @a[scores = {lh_uncuredDeath = 1..}, nbt = !{ActiveEffects:[{Id: 2b}]}] run effect give @s minecraft:slowness 1000000 1 true
execute as @a[scores = {lh_uncuredDeath = 1..}, nbt = !{ActiveEffects:[{Id: 2b}]}] run effect give @s minecraft:weakness 1000000 0 true
